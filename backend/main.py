...
async def login_for_access_token(
    ...
    try:
        ...
        # Если пользователь зарегистрирован через Google и у него нет пароля
        if db_user.auth_provider == "google" and not db_user.hashed_password:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Этот аккаунт использует Google для входа. Пожалуйста, войдите через Google.",
                headers={"WWW-Authenticate": "Bearer"},
            )
        ...
        # Если email не подтвержден и это не Google аккаунт
        if not db_user.is_active and db_user.auth_provider != "google":
            # Т.к. это не ошибка логина как таковая, не увеличиваем счетчик
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Пожалуйста, подтвердите ваш email перед входом. Проверьте вашу почту.",
                headers={"WWW-Authenticate": "Bearer"},
            )
        
        # Создаем данные для включения в JWT токен
        # sub - стандартный claim для хранения идентификатора пользователя в JWT
        token_data = {"sub": db_user.email}
        
        # Создаем JWT токен с ограниченным сроком действия
        # Используем expires_delta из настроек (по умолчанию 30 минут)
        access_token = create_access_token(data=token_data)
        
        # Не сбрасываем счетчик неудачных попыток - rate limiting отключен
        
        # Возвращаем созданный токен и его тип в соответствии со стандартом OAuth2
...
async def get_notifications(
    ...
    applications = (
        db.query(DoctorApplication)
        .filter(DoctorApplication.user_id == current_user.id)
...
async def mark_notification_as_read(
    ...
    if not existing_notification:
        # Если нет, создаем новую запись
        viewed_notification = ViewedNotification(
            user_id=current_user.id, application_id=notification_id
        )
        db.add(viewed_notification)
        db.commit()
